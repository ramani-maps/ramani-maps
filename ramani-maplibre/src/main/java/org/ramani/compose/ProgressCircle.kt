...
@Composable
fun ProgressCircle(center: LatLng,
                   radius: Float,
    ...
    Symbol(
        center = proj.fromScreenLocation(centerLocal + PointF(0.0f, -(radius + indicatorTextSize)*dpToPixel)),
    ...
    Circle(center = center, radius = radius, color = "Transparent", borderColor = borderColor, borderWidth = borderWidth)

    key(updateCounter.value) {
        when {
            progress.value == 100 -> {
                Circle(center = center, radius = radius, color = fillColor, borderColor = borderColor, borderWidth = borderWidth)
            }
                ...
                // arc
                Polyline(points = arcPoints, color = borderColor, lineWidth = borderWidth)

                Fill(points = arcPoints + center + arcPoints.first(), fillColor = fillColor)

                // connect arc with center on both sides to form the circle slice
                Polyline(
