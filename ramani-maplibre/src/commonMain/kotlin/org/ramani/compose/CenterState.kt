...
class CenterState(initialCenter: LatLng) {
    ...
    companion object {
        val Saver: Saver<CenterState, List<Double>> = Saver(
            save = { listOf(it.center.latitude, it.center.longitude, it.center.altitude) },
            restore = { CenterState(LatLng(it[0], it[1], it[2])) }
        )
    }
}
