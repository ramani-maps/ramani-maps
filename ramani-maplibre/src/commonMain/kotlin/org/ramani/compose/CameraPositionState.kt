 ...
 */
enum class CameraMotionType { INSTANT, EASE, FLY }
...
class CameraPositionState(initialPosition: CameraPosition = CameraPosition()) {
    ...
    companion object {
        val Saver: Saver<CameraPositionState, List<Any?>> = Saver(
            save = { state ->
                ...
                listOf(
                    ...
                    pos.bearing,
                    pos.motionType.name,
                    pos.animationDurationMs,
                )
            },
            restore = { list ->
                val target = if (list[0] != null) {
                    LatLng(list[0] as Double, list[1] as Double, list[2] as Double)
                } else null
                CameraPositionState(
                    CameraPosition(
                        target = target,
                        zoom = list[3] as Double?,
                        tilt = list[4] as Double?,
                        bearing = list[5] as Double?,
                        motionType = CameraMotionType.valueOf(list[6] as String),
                        animationDurationMs = list[7] as Int,
                    )
                )
            }
        )
